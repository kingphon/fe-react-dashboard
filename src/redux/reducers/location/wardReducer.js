...
const prefix = "WARD_";
// API
const PATH_API = `${REDUX_API_URL}/location/wards`;
...
export const fetchAllDistrict = (provinceId) => async (dispatch) => {
  return axios
    .get(`${REDUX_API_URL}/location/districts-creation/${provinceId}`, {
      timeout: 5000,
    })
...
export const doSave = (ward) => async (dispatch) => {
  ...
  console.log(ward);
  const { id, name, slugName, districtId, customizeSlug, status } = ward;
  const params = {
    name,
        ...
        return {
          ...
          ward: {
            ...
            provinceId: state.provinceList.find(
              (option) => option.value === action.ward.provinceId
            ),
            districtId: state.districtList.find(
              (option) => option.value === action.ward.districtId
            ),
          },
        };
